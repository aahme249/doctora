      ...
      <aside className={`fixed top-0 left-0 h-full w-64 bg-slate-900 text-white flex flex-col z-40 transform transition-transform duration-200
        ...
        <div className="px-4 py-4 border-t border-slate-700 space-y-3">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-teal-500 flex items-center justify-center text-sm font-bold shrink-0">
              {user?.name?.split(' ').map(n => n[0]).join('').slice(0, 2) ?? 'P'}
            </div>
            <div className="min-w-0">
              <p className="text-sm font-medium text-white truncate">{user?.name ?? 'Patient'}</p>
